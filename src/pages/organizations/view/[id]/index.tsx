...
import { deleteVanById } from 'apiSdk/vans';
...
function OrganizationViewPage() {
  ...
  const vanHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('van', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Vans
                    </Text>
                    <NextLink passHref href={`/vans/create?organization_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.van?.map((record) => (
                          <Tr cursor="pointer" onClick={() => router.push(`/vans/view/${record.id}`)} key={record.id}>
                            <Td>{record.name}</Td>
